...
核验日期：2026-09-06；2026-09-07 补充上游核对。用户已明确以 `archive/RCBldEng-v1.3.0.zip` 为后续重构基准。本文记录新版的直接证据；[原始调查](evidence.md) 中关于旧 EXE 的接口、算法和缺陷只能作为历史线索。资产登记见 [DATA.md](../_dev/DATA.md)。

> **上游核对更新（2026-09-07）**：已按用户指定的 `andersonspy/RCBIdEng` 固定提交读取 README、8 个 `.sim` 和 25 个历史 CSV，并确认正式 v1.3.0 下载资产与本地 ZIP 指纹一致。仓库文件树 EXE 不同且依赖不完整，不能替代 ZIP；仍无 `.sol` 或可读业务源码。详见 [上游协议核对](engine-protocol.md)。以下 2026-09-06 静态分析边界保持，资料缺口以新增记录为准。

> **反编译核查更新（2026-09-07）**：已扩大检查至全部 PYZ 条目，补查 `comfort`，并核查 3 个正式发行包及上游全部 9 次可达提交，共覆盖 5 份不同 EXE；仍未恢复未保护的业务实现。直接证据、历史指纹和后续取证路径见 [反编译与源码恢复核查](engine-recovery.md)。

> **Linux 静态恢复更新（2026-09-07）**：使用用户指定的 reverse-skill 流程，已恢复 5 个业务模块的 196 个代码对象反汇编，确认入口固定一阶、`cwd` 父目录定位及部分解析/太阳覆盖行为。完整证据见 [静态逆向报告](artifacts/engine-v1.3.0/report/recovery.md)。以下初次分析中的“业务函数不可读”“新版未知”按当时状态保留；当前缺口主要为完整协议、可运行源码与原版数值验证。
...
2026-09-07 已下载上游 v1.3.0 正式发行资产并核对 SHA-256，与用户文件完全一致，版本身份因此有正式发行记录支持；未通过程序内部版本命令确认。上述指纹限定结论对象，不能仅凭仓库标签或文件名替换二进制。
...
| ADR-009：`.sol` 按热区提供窗及不透明围护太阳序列 | 用户说明 v1.3 有意关闭 `.sol` 读取；顺序、单位、长度已由 1.1 源码确认（[sol-protocol.md](sol-protocol.md)），作为引擎重开后的目标契约。 |
| ADR-003、ADR-012、ADR-013：输出列、非空调耦合、非正交/坡面表达 | 产品需求保持；旧算法及接口证据不能替代新版能力验收。 |
| 旧 `simulation.py:177` 的 `q_sol_op == Solar_OP` | 1.1 源码确认为比较表达式，`Solar_OP` 从未生效；引擎重开 `.sol` 时须修复。 |
| 旧 `solarGain` 依据首个热区有无 `Solar_OP` 分支 | 1.1 源码确认（`simulation.py:87-88`）；引擎重开时改为逐区判断，并保留“仅后续热区提供太阳输入”的控制案例。 |
