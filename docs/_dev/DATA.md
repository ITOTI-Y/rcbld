...
原始 35 列按 CSV 字符串保存；离线工具读取前 5 列时间字段及观测指示/天气代码，并检查干球、露点、湿度、气压、水平红外、总水平/直射法向/散射水平辐射、风向、风速及两类云量。类型、单位、缺测阈值与范围的历史定义保留在 Git 提交 `2e88691` 的 `docs/_dev/03_WEATHER_IMPL.md` 中；该设计文档已于 2026-09-08 按用户要求删除。源文件不改写、不填零；参考年只用于检查月/日/小时覆盖，不选择模拟日历。其他列及完整头字段尚未实施语义校验。
...
## output-rooms-implementation

- **path**: `docs/_dev/`
- **format**: Markdown，包含完整 Python 脚本和 GH 端口、连线说明
- **source**: 用户于 2026-09-08 确认从 Rhino 操作流程重新设计，并批准先交付热区接入与检查；接口依据 McNeel 文档和本机 Rhino 8.25 SDK
- **purpose**: 供用户手动实现首个热区组件，并据使用流程安排后续功能
- **flow**: `approved Rhino workflow + API inspection -> manual implementation documents -> user-created grasshopper/rooms.py and examples/rooms.gh`
- **status**: active
- **quality**: unverified
- **created**: 2026-09-08
- **updated**: 2026-09-08
- **notes**: 本条仅覆盖 `01_WORKFLOW_IMPL.md`、`02_ROOMS_IMPL.md`。文档代码已在临时 Python 3.9.11 环境通过模拟接口的行为检查；Ruff 和基于临时 SDK 接口声明的 ty 检查通过。原生 Rhino 几何、视口与跨组件对象传递尚未运行验证。业务文件及 GH 定义尚未创建，临时验证文件不作为持久资产登记。

### Schema

`01_WORKFLOW_IMPL.md` 说明从接入热区到保存比选的使用顺序、本轮边界与 `Room` 字段。`02_ROOMS_IMPL.md` 包含一个完整 Python 代码块，对应 `grasshopper/rooms.py`，以及手动创建 `examples/rooms.gh` 的端口与连线说明。代码块采用 Python 3.9.11 语法，在 GH Python 3 普通脚本模式执行；输入为两个 Flatten 的列表，输出为热区、整组状态、两组预览几何、标签位置、标签和报告。预览保持模型单位，计算副本使用 m，统计量使用 m²、m³。
