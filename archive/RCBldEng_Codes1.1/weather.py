...
class Weather():
    ...
    def getWeather(self):
        ...
        wea_dict = {}
        with open(self.wea_file, 'r') as fweather:
        ...
        wea_dict['T_er'] = (wea_dict['HIR']/(5.6697*10**-8))**.25-273.15
        self.wea_dict = wea_dict

    def cal_Ydate(self):
        '''calculate date number of year'''
        Ydate = [1]
        month = self.wea_dict['month']; day = self.wea_dict['day'];
...
#-----------------------------------Direct, diffuse, and ground reflection radiation calculation
    ...
    def cal_ET(self):
        ...
        self.cal_dict['ET'] = 2.2918*(0.0075+0.1868*np.cos(tao)-3.2077*np.sin(tao)-1.4615*\
                              np.cos(2*tao)-4.089*np.sin(2*tao))

    def cal_AST(self):
        '''calculate apparent solar time'''
        ET = self.cal_dict['ET']
        self.cal_dict['AST'] = self.wea_dict['hour'] + ET/60 + (self.longitude-self.LSM)/15
    ...
    def cal_solarDegree(self):
        ...
        self.cal_dict['delta'] = 23.45*np.sin((self.cal_dict['Ydate']+284.0)/365*2*np.pi)
    ...
    def cal_azimuth(self):
        ...
        sin_beta = np.cos(self.latitude*np.pi/180)*np.cos(delta*np.pi/180)*np.cos(theta_h*np.pi/180)+\
        np.sin(self.latitude*np.pi/180)*np.sin(delta*np.pi/180)
        ...
        sin_fai = np.sin(theta_h*np.pi/180)*np.cos(delta*np.pi/180)/np.cos(beta_rad)
        self.cal_dict['sin_fai'] = sin_fai
        cos_fai = (np.cos(theta_h*np.pi/180)*np.cos(delta*np.pi/180)*np.sin(self.latitude*np.pi/180)-np.sin(delta*np.pi/180)*np.cos(self.latitude*np.pi/180))/np.cos(beta_rad)
        ...
        fai_deg = np.where(sin_fai>0, fai2_deg, np.where(cos_fai>0, fai1_deg, -180-fai1_deg))
    ...
    def cal_surface(self):
        ...
        self.cal_dict['Et_r'] = (self.wea_dict['Eb']*np.sin(beta_rad)+self.wea_dict['Ed'])*\
                                r_g * (1 - np.cos(sigma*np.pi/180))/2
        surface = lib.Surface()
        for attr, value in surface.iterAttributes():
            self.cal_dict['surf'][attr] = {}
            self.cal_dict['surf'][attr]['gamma'] = np.absolute(fai_deg - value)
            self.cal_dict['surf'][attr]['cos_theta'] = np.cos(beta_rad)*np.cos(self.cal_dict['surf'][attr]['gamma']*np.pi/180)*\
                                                  np.sin(sigma*np.pi/180)+np.sin(beta_rad)*np.cos(sigma*np.pi/180)
            self.cal_dict['surf'][attr]['theta'] = np.arccos(self.cal_dict['surf'][attr]['cos_theta'])*180/np.pi
            gamma = self.cal_dict['surf'][attr]['gamma']
            cos_theta = self.cal_dict['surf'][attr]['cos_theta']
            self.cal_dict['surf'][attr]['Et_b'] = np.where((gamma>90) & (gamma<270), 0, np.where(cos_theta<0, 0, self.wea_dict['Eb']*cos_theta))
            self.cal_dict['surf'][attr]['Y'] = 0.55+0.437*cos_theta+0.313*(cos_theta**2)
            self.cal_dict['surf'][attr]['Y'] = np.where(self.cal_dict['surf'][attr]['Y']<0.45, 0.45, self.cal_dict['surf'][attr]['Y'])
            Y = self.cal_dict['surf'][attr]['Y']            
            self.cal_dict['surf'][attr]['Et_d'] = np.where(sigma<=90, self.wea_dict['Ed']*(Y*np.sin(sigma*np.pi/180)+np.cos(sigma*np.pi/180)), self.wea_dict['Ed']*(Y*np.sin(sigma*np.pi/180)))
            self.cal_dict['surf'][attr]['GSR'] = self.cal_dict['surf'][attr]['Et_b']+self.cal_dict['surf'][attr]['Et_d']+self.cal_dict['Et_r']
...
#----------------------------------------------------------------------specific humidity calculation
    def cal_mmv(self):
        ...
        water = [6.11E+00,4.44E-01,1.43E-02,2.65E-04,3.03E-06,2.03E-08,6.14E-11]
        ice = [6.11E+00,5.03E-01,1.89E-02,4.18E-04,5.82E-06,4.84E-08,1.84E-10]
    ...
    def cal_SRF(self):
        ...
        for i in range(15, 61, 15):
            ...
            for orient, psi in surface.iterAttributes():
                ...
                self.cal_dict['overhang'][i][orient] = np.where(GSR == 0, 0, (np.maximum(0, 1-(0.5*np.tan(np.deg2rad(i))/np.tan(np.deg2rad(90.0-np.maximum(beta_deg,0)))))*Et_b+(1-i/90.0)*Et_d+Et_r)/GSR)
                if orient == 'S' or orient == 'SE' or orient == 'E' or orient == 'NE':
                    self.cal_dict['fin'][i][orient] = np.where(GSR==0, 0, np.where((fai_deg-psi>90) | (fai_deg-psi<0), 1, ((np.maximum(0, 1-0.5*np.tan(np.deg2rad(i))/np.tan(np.deg2rad(90.0-np.absolute(fai_deg-psi)))))*Et_b+Et_d+Et_r)/GSR))
                else:
                    self.cal_dict['fin'][i][orient] = np.where(GSR==0, 0, np.where((fai_deg-psi<-90) | (fai_deg-psi>0), 1, ((np.maximum(0, 1-0.5*np.tan(np.deg2rad(i))/np.tan(np.deg2rad(90.0-np.absolute(fai_deg-psi)))))*Et_b+Et_d+Et_r)/GSR))
        ...
        for i in range(10, 81, 10):
            ...
            for orient, psi in surface.iterAttributes():
                ...
                Et_b = self.cal_dict['surf'][orient]['Et_b']
                Et_d = self.cal_dict['surf'][orient]['Et_d']                
                self.cal_dict['horizon'][i][orient] = np.where(GSR==0, 0, np.where(np.maximum(beta_deg, 0)<i, 1-Et_b/GSR,1))
    ...
    def cal_Esol(self):
        ...
        for tilt in range(15, 91, 15):
            ...
            for orient, psi in surface.iterAttributes():
                ...
                self.cal_dict['Esol'][tilt][orient] = incident*(np.cos(beta_rad)*np.sin(np.deg2rad(tilt))*np.cos(np.deg2rad(fai_deg-psi))+np.sin(beta_rad)*np.cos(np.deg2rad(tilt)))
                self.cal_dict['Esol'][tilt][orient] = np.where((self.cal_dict['Esol'][tilt][orient]<0) | (self.cal_dict['Esol'][tilt][orient]>=1500), 0, self.cal_dict['Esol'][tilt][orient])
...
#-------------------------------------------------------------------------dump results into file
    def dumpResults(self):
        ...
        for surf in list(self.cal_dict['surf'].keys()):
            output_dict[surf+' orientation global solar radiation'] = self.cal_dict['surf'][surf]['GSR']
