...
def baseCEN(building, first_day, runcode, **kwargs):
    ...
    if os.path.isfile(file_path):
        ...
        if not first_char:
            ...
            with open(
                run_folder + building + "_" + runcode + ".sol", encoding="utf-8"
            ) as f_solar:
            ...
            for i in range(len(content)):
                if ":" in content[i]:
                    ...
                    input_dict["Zones"][content[i].split(":")[0]]["Solar_OP"] = (
                        np.array(
                            content[i].split(":")[1].split(";")[1].split(",")
                        ).astype(float)
                    )
                    input_dict["Zones"][content[i].split(":")[0]]["Solar_W"] = np.array(
                        content[i].split(":")[1].split(";")[0].split(",")
                    ).astype(float)
    ##
