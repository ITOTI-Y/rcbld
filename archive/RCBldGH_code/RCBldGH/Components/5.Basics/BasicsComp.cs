...
{
    public class BasicsComp:GH_Component
    {
        public BasicsComp(): base("Basics", "Basics", "Building information","RCBldGH", "5.Basics")
        {
        }

        public override Guid ComponentGuid => new Guid("{835360EF-C622-4247-BF0E-3CC1CA927E7E}");
        ...
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            ...
            pManager.AddIntegerParameter("Floors", "FN", "Number of floors", GH_ParamAccess.item);
            pManager.AddNumberParameter("Building Height", "H", "Building Height. Uint: m", GH_ParamAccess.item);
            pManager.AddNumberParameter("Building Length", "L", "Building Length. Unit: m", GH_ParamAccess.item);
            pManager.AddNumberParameter("Building Width", "W", "Building Length. Unit: m", GH_ParamAccess.item);
            pManager.AddNumberParameter("Ground Floor Area", "A", "Building Area. Unit: m2", GH_ParamAccess.item);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            string groundSurfaceTemperatureStr = string.Empty;
            try
            {
                //groundSurfaceTemperatureStr = ReadTemperatureFromEpw(path);
            ...
            catch (Exception e)
            {
                ...
                // 这里不用 return ，下一行会判断是否读到了 Ground Surface Temperature。
            }
            ...
            int floors = -1;
            if (!DA.GetData("Floors", ref floors))
            {
                return;
            }

            double height = double.NaN;
            if (!DA.GetData("Building Height", ref height))
            {
                return;
            }

            double length = double.NaN;
            if (!DA.GetData("Building Length", ref length))
            {
                return;
            }

            double width = double.NaN;
            if (!DA.GetData("Building Width", ref width))
            {
                return;
            }

            double area = double.NaN;
            if (!DA.GetData("Ground Floor Area", ref area))
            {
                return;
            }
