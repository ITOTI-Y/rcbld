...
{
    public class NEWZoneComp1 : GH_Component
    {
        public NEWZoneComp1()
            : base("Zone Creator1.1", "Zone Creator","Zone Creator","RCBldGH", "2.Envelops")
        {
        }
        public override GH_Exposure Exposure => GH_Exposure.tertiary;
        public override Guid ComponentGuid => new Guid("{1da6a6b8-20f9-40f9-93b2-3e0bbc171a7e}");
        ...
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("SolarOP", "C", "Is the zone closed?", GH_ParamAccess.list);
            pManager[5].Optional = true;
            pManager.AddNumberParameter("SolarW", "AF", "AF", GH_ParamAccess.list);
        ...
        protected override void RegisterOutputParams(GH_OutputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("solar_W", "S", "solar", GH_ParamAccess.list);
            pManager.AddNumberParameter("solar_OP", "S", "solar", GH_ParamAccess.list);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (DA.GetDataList(5, Solar))
            {
                zone.SolarOP = Solar.ToArray().Select(x => x / zone.Area).ToArray();
            }
            else
            {
                zone.SolarOP = null;
            ...
            List<double> SolarInput = new List<double> { };
            if (DA.GetDataList(6, SolarInput))
            {
                try
                {
                    zone.SolarW = SolarInput.Select(x => x / zone.Area).ToArray();
                }
                catch (Exception)
                {
                    AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,"b");
                }
            }
            else
            {
            ...
            DA.SetDataList(2, zone.SolarW);
            DA.SetDataList(3, zone.SolarOP);
        }
