...
{
    public class MaterialAssignNewComp : GH_Component
    {
        public MaterialAssignNewComp() : base("MaterialAssignNew", "Material to Brep", "Assignment MaterialSetting to breps", "RCBldGH", "2.Envelops")
        {
        }
        public override GH_Exposure Exposure => GH_Exposure.primary;
        public override Guid ComponentGuid => new Guid("{bdabc093-1ffd-4934-ad01-8f5a6349cb6a}");
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            Parallel.For(0, rooms.Count, i =>//建筑
            {
            ...
            Parallel.For(0, prTree.BranchCount,i =>//每个房间
            {
                int iAsInt = (int)i;
                for (int j = 0; j < prTree.Branch(prTree.Paths[iAsInt]).Count;j++)
                {
                    
                    if (prTree[prTree.Paths[iAsInt], j].RelativePosition == 0)//若为顶面
                    {
                        bool isExter = true;
                        for (int k = 0; k < buildingBottoms.Count; k++)
                        {
                            buildingBottoms[k].GH_Surface.Value.Faces[0].ClosestPoint(prTree[prTree.Paths[iAsInt], j].Centroid, out double u, out double v);
                            Point3d point = buildingBottoms[k].GH_Surface.Value.Faces[0].PointAt(u, v);
                            if (prTree[prTree.Paths[iAsInt], j].Centroid.DistanceTo(point) < 0.00001)
                            {
                                prTree[prTree.Paths[iAsInt], j].Material = materialSetting.InternalFloorMaterial;
                                buildingBottoms[k].Name = prTree[prTree.Paths[i], j].Name;
                                isExter = false;
                                break;
                            }
                        }
                        if (isExter)
                        {
                            prTree[prTree.Paths[    iAsInt], j].Material = materialSetting.RoofMaterial;
                        }
                    }//top

                    if (prTree[prTree.Paths[iAsInt], j].RelativePosition == 1)//若为墙面
                    {
                        bool isExter = true;
                        int exceptitself = 0;
                        for(int k=0; k<buildingWalls.Count; k++)
                            {
                                buildingWalls[k].GH_Surface.Value.Faces[0].ClosestPoint(prTree[prTree.Paths[iAsInt], j].Centroid, out double u, out double v);
                                Point3d point = buildingWalls[k].GH_Surface.Value.Faces[0].PointAt(u, v);
                                if (prTree[prTree.Paths[iAsInt  ], j].Centroid.DistanceTo(point) < RhinoMath.ZeroTolerance)
                                {
                                   buildingWalls[k].Name = prTree[prTree.Paths[i], j].Name;
                                   exceptitself += 1;
                                }
                        ...
                        if (exceptitself >= 2)
                        {
                            prTree[prTree.Paths[iAsInt], j].Material = materialSetting.InternalWallMaterial;
                            isExter = false;
                            
                        }
                        if (isExter)
                        {
                            prTree[prTree.Paths[iAsInt], j].Material = materialSetting.ExternalWallMaterial;
                        }
                    }//wall

                    if (prTree[prTree.Paths[iAsInt], j].RelativePosition == 2)//若为底面
                    {
                        bool isExter = true;
                        for (int k = 0; k < buildingTops.Count; k++)
                        {
                            buildingTops[k].GH_Surface.Value.Faces[0].ClosestPoint(prTree[prTree.Paths[iAsInt], j].Centroid, out double u, out double v);
                            Point3d point = buildingTops[k].GH_Surface.Value.Faces[0].PointAt(u, v);
                            if (prTree[prTree.Paths[iAsInt], j].Centroid.DistanceTo(point) < 0.00001)
                            {
                                prTree[prTree.Paths[iAsInt], j].Material = materialSetting.InternalFloorMaterial;
                                buildingTops[k].Name = prTree[prTree.Paths[i], j].Name;
                                isExter = false;
                                break;
                            }
                        }
                        if (isExter)
                        {
                            prTree[prTree.Paths[iAsInt], j].Material = materialSetting.ExternalFloorMaterial;
                            prTree[prTree.Paths[iAsInt ], j].IsSlab= true;
                        }
                    }//bottom
                } ;
            ...
            Parallel.For(0, windows.Count, k =>
            {
