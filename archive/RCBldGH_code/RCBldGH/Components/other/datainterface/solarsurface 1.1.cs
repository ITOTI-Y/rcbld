...
{
    public class SolarSurfaceComp : GH_Component
    {
        public SolarSurfaceComp() : base("Solarsurface1.1", "Material to Brep", "Assignment MaterialSetting to breps", "RCBldGH", "Other")
        {
        }
        public override GH_Exposure Exposure => GH_Exposure.primary;
        public override Guid ComponentGuid => new Guid("{6e8d8411-8365-4312-ae31-532bfe812a80}");
        ...
        protected override void RegisterOutputParams(GH_OutputParamManager pManager)
        {
            ...
            pManager.AddGenericParameter("suface", "setting", "Envelope setting", GH_ParamAccess.tree);
            
            
        }
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            GH_Structure <GH_Surface> SolarSurfaces = new GH_Structure<GH_Surface> { };
            ...
            DataTree<GH_Surface> RadSurface = new DataTree<GH_Surface>();
                ...
                for (int j = 0; j < prTree.Branch(prTree.Paths[i]).Count; j++)//每个面
                {
                    if (prTree[prTree.Paths[i], j].RelativePosition == 0)//若为顶面
                    {
                        ...
                        if (isExter)
                        {
                            ...
                            RadSurface.Add(prTree[prTree.Paths[i], j].GH_Surface, prTree.Paths[i]);
                        }
                    }//top
                    ...
                    if (prTree[prTree.Paths[i], j].RelativePosition == 2)//若为底面
                    {
                        ...
                        {
                            buildingTops[k].GH_Surface.Value.Faces[0].ClosestPoint(prTree[prTree.Paths[i], j].Centroid, out double u, out double v);
